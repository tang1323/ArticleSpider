...
# pipelines可以拦截items的数据,然后可以将数据保存到你想要的地方,如mysql
...
class ArticleImagePipeline(ImagesPipeline): # 只处理封面图片
    def item_completed(self, results, item, info):
        if "front_image_url" in item:# 判断如果没有封面图片时不执行以下语句,有就执行,好比知乎没有封面图片
            image_file_path = ""
